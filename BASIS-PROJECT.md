...
| Retry premium | Empirical failure statistics grouped by explicit deterministic cause codes |
...
│                                                                        │
│  Any typed HTTP or MCP client        @basis/sdk      │
│                │                                         │             │
...
╚══════════════════════════════╪══════════════════════════════════════════╝
                   ...
                   ▼                        ▼
       ┌──────────────────────┐  ┌─────────────────────────────────────┐
       │ Public dashboard     │  │ Deterministic reporter: read-only          │
       │                      │  │                                     │
       │ open execution book  │  │ state-transition audit             │
       │ P&L and hit rate     │  │ explicit failure-code counts              │
...
| Reconciliation reporter | No execution credential | **No** | Reads exported ledger evidence only |
| Executor | KeeperHub write scope | **Yes** | Deterministic state-machine service |
...
> **Only the deterministic executor holds KeeperHub write credentials; no model provider participates in runtime.**
...
## 13. Deterministic reconciliation and reporting

Basis uses a deterministic reconciliation worker and a separate read-only report. Both apply explicit rules to SQLite records, hash-chained events, KeeperHub status, and independent RPC receipts. They do not call a model provider.

The worker may advance only transitions allowed by the canonical state machine. It polls the original KeeperHub execution ID for uncertain outcomes and never rebroadcasts from `UNCERTAIN`. The report computes counts and alerts from stored events without credentials or write authority.

An optional future state-transition audit may only convert plain text into an untrusted `JobProposal`. It is disabled by default, has no credentials or ledger access, and every proposal must pass the normal strict JSON quote path without automatic repair.
...
| Reconciliation | Deterministic TypeScript worker and read-only report | Explicit status checks, recovery, and alerts |
...
- No model provider in pricing, payment, execution, settlement, verification, reconciliation, reporting, or refund decisions
...
The deterministic reconciliation worker runs inside Basis with narrowly scoped KeeperHub status access. The standalone report reads exported ledger evidence and has no credentials.
...
- Deterministic reconciliation worker and read-only report
- Failure triage
...
| Untrusted proposal is malformed or unsafe | Dangerous job request | Strict schema, registry and allowlist validation; reject without automatic repair |
...
- Deterministic reconciliation worker and read-only report with reconciliation report
...
- "Model-based pricing"—the price is deterministic arithmetic.
